...
)
{
    ...
    snprintf(
        ...
        "{\"1\":%f,\"5\":%f,\"11\":%f,\"2\":%f}", 
        temp1, temp2, temp3, humi
    );
    err = report_model_data(resp, data_string, rt_strlen(data_string));
    // err = report_model_data(resp, "{\"1\":22.5}", rt_strlen("{\"1\":22.5}"));
...
extern rt_err_t nbiot_reset_pin_enable(rt_uint8_t mode);
extern rt_err_t bc660k_boot_pin_enable(rt_uint8_t mode);
...
{   
    ...
    res = iot_psmeint_pin_enable(PIN_HIGH);
    LOG_D("iot_psmeint_pin_enable HIGH res: %d", res);
    // res = nbiot_reset_pin_enable(PIN_LOW);
    // LOG_D("nbiot_reset_pin_enable LOW res: %d", res);
    // res = bc660k_boot_pin_enable(PIN_LOW);
    // LOG_D("bc660k_boot_pin_enable LOW res: %d", res);
    res = antenna_active_pin_enable(PIN_HIGH);  // power on antenna
    LOG_D("antenna_active_pin_enable HIGH res: %d", res);
    res = sensor_pwron_pin_enable(PIN_HIGH);
    LOG_D("sensor_pwron_pin_enable HIGH res: %d", res);
    res = sim_select_pin_enable(PIN_LOW);  // low for sim1
    LOG_D("sim_select_pin_enable LOW res: %d", res);
    res = nb_cat1_rf_pin_enable(PIN_LOW);  // low for nbiot rf
    LOG_D("nb_cat1_rf_pin_enable LOW res: %d", res);
    res = intn_ext_ant_pin_enable(PIN_LOW);
    LOG_D("intn_ext_ant_pin_enable LOW res: %d", res);
    res = flash_pwron_pin_enable(PIN_HIGH);
    LOG_D("flash_pwron_pin_enable HIGH res: %d", res);
