...
{   
    ...
    for (int i=0; i < 3; i++) {
        result = at_obj_exec_cmd(client, resp, at_string);
        if (result == RT_EOK) {
            break;
        }
        rt_thread_mdelay(20);
...
rt_err_t report_model_data(const char *data, rt_size_t length)
{
    ...
    err = nbiot_qiotmodeltd_set(resp, 0, length, -1);
    if (err != RT_EOK) {
    ...
    return at_client_obj_send(client, data, length);
}
